var PDB = new function(){
    ...
    var _linkedList = function(colection){
        ...
        var _remove = function(d){
            var n;
            if(typeof d === "object") n = _searchFirstWhere(d);
            else n = _searchFirst(d);
            ...
            if(n == lnode){
                ...
                _findByIterator(count-1).next = n.next;
        ...
        var _import = function(c){
            ...
            for(var e in cd) if(cd[e].length > 0) _push((cd[e][0] != "{" && cd[e][0] != "[")?cd[e] : JSON.parse(cd[e]));
